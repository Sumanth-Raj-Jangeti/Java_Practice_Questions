...
abstract class Employee {
    ...
    public Employee(String employeeName){
        ...
        if(this instanceof ContractEmployee){
            ...
            permenantIdCounter++;
            setEmployeeId("E" + permenantIdCounter);
        }
    }
    ...
    public void setEmployeeName(String employeeName) {
        if (employeeName != null && employeeName.matches("^[A-Z][a-zA-Z]+(\\s[A-Z][a-zA-Z]+)+$"))
...
class PermanentEmployee extends Employee{
    ...
    private Asset[] assets;  //PermanentEmployee has-a asset
    ...
    public double calculateBonus(float experience) throws InvalidExperienceException {
        if(experience < 2.5) throw new InvalidExperienceException("A minimum of 2.5 years is required for bonus!");
        ...
        if(salaryComponents!=null){
            for(String component:salaryComponents){
                if(component.startsWith("DA-")){
                    da=basicPay*Integer.parseInt(component.split("-")[1])/100;
                }else if(component.startsWith("HRA-")){
                    hra=basicPay*Integer.parseInt(component.split("-")[1])/100;
                }
            }
        }
        //Invoking calculateBonus()
        try{
            bonus=calculateBonus(experience);
        }catch(InvalidExperienceException e){
            bonus=0;
        }
        double salary = basicPay+da+hra+bonus;
        setSalary(Math.round(salary));
    }
    public Asset[] getAssetsByDate(String lastDate) throws InvalidAssetsException {
        ...
        String[] expiryParts = lastDate.split("-");
        int expiryYear  = Integer.parseInt(expiryParts[0]);
        int expiryMonth = Resources.getMonth(expiryParts[1]);
        int expiryDay   = Integer.parseInt(expiryParts[2]);

        Asset[] result = new Asset[assets.length];   // same length as input
        int idx = 0;                                 // fill from front

        for (Asset asset : assets) {
            if (asset == null || asset.getAssetExpiry() == null) continue;

            String[] p = asset.getAssetExpiry().split("-");
            int y = Integer.parseInt(p[0]);
            int m = Resources.getMonth(p[1]);
            int d = Integer.parseInt(p[2]);

            if (y < expiryYear
                    || (y == expiryYear && m < expiryMonth)
                    || (y == expiryYear && m == expiryMonth && d <= expiryDay)) {
                result[idx++] = asset;     // <-- pack at front
            }
        }

        if (idx == 0) throw new InvalidAssetsException("No assets found for the given criteria!");
        return result;                     // trailing slots remain null
    }
    @Override
    public String toString() {
...
class Admin {
    ...
    public int generateAssetsReport(Employee[] employees,String lastDate){
        if (employees == null || lastDate == null) return -1;
        int count = 0;
        for (Employee emp : employees) {
            if (emp instanceof PermanentEmployee) {
                try {
                    Asset[] assets = ((PermanentEmployee) emp).getAssetsByDate(lastDate);
                    for (Asset a : assets) {
                        if (a != null) count++;   // skip nulls
                    }
    ...
    public String[] generateAssetsReport(Employee[] employees, char assetCategory){
        ...
        for (Employee emp : employees) {
            if (emp instanceof PermanentEmployee) {
                Asset[] assets = ((PermanentEmployee) emp).getAssets();
                if (assets != null) {
                    for (Asset asset : assets) {
                        if (asset != null && asset.getAssetId() != null) {
                            String id = asset.getAssetId();
                            if ((assetCategory == 'D' || assetCategory == 'd') && id.startsWith("DSK")) {
                                result.add(id);
                            }
                            else if ((assetCategory == 'L' || assetCategory == 'l') && id.startsWith("LTP")) {
                                result.add(id);
                            }
                            else if ((assetCategory == 'I' || assetCategory == 'i') && id.startsWith("IPH")) {
                                result.add(id);
                            }
                        }
                    }
